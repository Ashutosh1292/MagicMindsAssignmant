...
class AddProduct(GenericAPIView):
    ...
    async def post(self,request):
        try:
            ...
            procucts = await sync_to_async(Products(name=name,description=description,price=price,category=category,inventory_count=inventory_count))
            await sync_to_async(procucts.save)()
...
class GetProduct(GenericAPIView):
    ...
    async def get(self, request):
        try:
            products = await sync_to_async(Products.objects.all)()
            products_list = list(await sync_to_async(products.values)(
...
class SearchProduct(GenericAPIView):
    ...
    async def post(self, request):
        try:
            ...
            products =await sync_to_async( Products.objects.filter(
                Q(name__icontains=keyword) | Q(description__icontains=keyword) |  Q(category__icontains=keyword)
            ))
            products_list =await sync_to_async( list(products.values("id", "name", "description", "price", "category", "inventory_count")))
            response = {
                "status": 200,
...
class AddSales(GenericAPIView):
    ...
    async def post(self, request):
        try:
            ...
            print(product)
            sale_log = await sync_to_async(SaleLog.objects.create(product_id=product, sale_quantity=quantity))
            print(sale_log)
...
class GetProductScore(GenericAPIView):
    ...
    async def get(self,request):
        try:
            ...
            products = await sync_to_async(Products.objects.annotate(total_sale_quantity=Sum('salelog__sale_quantity')).order_by('-total_sale_quantity') )
